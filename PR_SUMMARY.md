...
*   **Source Code Download Mechanisms**: Packaged `single_cycle_cpu.zip` and `multi_cycle_cpu.zip` physical archives. Injected instant point-and-click download links directly at the bottom of the Week 10 curriculum so students can effortlessly grab and modify the SystemVerilog code locally.
*   **Pedagogical UX Improvements**: Re-sequenced the `notes_week_10.md` reading flow. Evaluated "Execution" vs "Download" logic, permanently shifting the SystemVerilog/zip retrieval step *before* the terminal `make` instructions, ensuring students inherently deploy the local environment prior to executing compilation targets.
