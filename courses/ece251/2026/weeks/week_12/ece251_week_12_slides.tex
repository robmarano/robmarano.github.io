...
\begin{itemize}
    ...
    \item \textbf{Step 1: Run the Automated Makefile} – The \texttt{Makefile} automatically assembles the MIPS instructions, synthesizes the SystemVerilog netlist, and dynamically executes the simulation natively:
\end{itemize}
\begin{verbatim}
$ make clean all ASM=test_prog
\end{verbatim}
\begin{itemize}
    \item The testbench internally hooks the binary payload bounds directly into active memory arrays cleanly.
    \item The testbench additionally generates a digital execution wave file locally: \texttt{tb\_computer.vcd}.
\end{itemize}
\end{frame}

\begin{frame}[fragile]{15. Simulating the Processor (Part 2: Text Trace)}
\begin{itemize}
    \item \textbf{Step 2: Trace Cycle-by-Cycle Execution} – The simulation quietly diverts all console outputs directly into a tracked diagnostic file natively!
\end{itemize}
\begin{verbatim}
$ cat debug_output.txt
\end{verbatim}
\begin{itemize}
    \item The text output mathematically traces the exact hexadecimal state of every boundary stage (\texttt{[IF]}, \texttt{[ID]}, \texttt{[EX]}, \texttt{[MEM]}, \texttt{[WB]}) dynamically!
    \item We can visually read precisely when \texttt{FlushD/FlushE} sequence pins correctly isolate structural limits directly from the terminal without ever rendering digital logic wave models visually!
\end{itemize}
\end{frame}
