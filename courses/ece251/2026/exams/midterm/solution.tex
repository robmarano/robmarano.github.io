...
\begin{minipage}{0.65\textwidth}
    \raggedleft {\bf Name: } {\color{red} \textbf{ANSWER KEY}}
\end{minipage}
...
\begin{itemize}
    \item \textbf{Scheduling:} Email \underline{and} direct message me on Teams by \textbf{Friday, March 13 (tomorrow)} to lock in your testing slot. The testing window is from March 13 through \textbf{Wednesday, March 25}.
    \item \textbf{Taking the Exam:} At your scheduled time, I will send the PDF. You have exactly \textbf{90 minutes} (approved accommodations apply) to complete 6 problems (100~pts), closed-book, using only the attached MIPS Green Sheet. Provide clear, brief solutions with explicit assumptions. Show all work and \textbf{box} final answers. 
...
\begin{enumerate}[\bf 1.]
    ...
    {\color{red} \textbf{12 GHz}}
    \vspace{0.5in}
    
    \item \textbf{General Computer Architecture Concepts (15 pts):} \\
    (a) \textit{[See structural diagram on problem page.]} \\
    (b) {\color{red} Programs and instructions share the same memory layout.} \\
    (c) {\color{red} 1. Simplicity favors regularity 2. Smaller is faster 3. Make common case fast 4. Good design demands good compromises} \\
    (d) {\color{red} Big-Endian: MSB at lowest address. Little-Endian: LSB at lowest address.}
    \vspace{0.5in}
    
    \item \textbf{MIPS Architecture \& Layout (15 pts):} \\
    (a) {\color{red} Variables map physically via explicit load \& store calls; ALU isolated strictly to registers.} \\
    (b) {\color{red} \textbf{R-Type:} opcode/rs/rt/rd/shamt/funct. \textbf{I-Type:} opcode/rs/rt/imm. \textbf{J-Type:} opcode/target.} \\
    (c) {\color{red} \texttt{.text} contains physical logic at \texttt{0x00400000}. \texttt{.data} contains arrays at \texttt{0x10000000}.} \\
    (d) {\color{red} Base offset maps static register + imm bytes. PC-Relative jumps exactly \texttt{PC + (imm * 4)}.}
    \vspace{0.5in}
    
    \item \textbf{Assembly Compilation and Disassembly (20 pts):} \\
    (a) \textit{Machine Code Hex:} {\color{red} \textbf{0x8E69FFFC}} \hfill (b) \textit{Assembly Instruction:} {\color{red} \textbf{sw \$t3, 8(\$s0)}}
    \vspace{0.5in}
    
    \item \textbf{Translating a C array manipulation (Leaf Procedure) (15 pts):} \\
    {\color{red} \textit{See full assembly MIPS sequence on problem page limit space.}}
    \vspace{0.5in}
    
    \item \textbf{Translating Nested Procedures (Stack Pointer Management) (20 pts):} \\
    {\color{red} \textit{See full push/pop nested factorial tree execution on problem page.}}
    \vspace{0.5in}
\end{enumerate}
