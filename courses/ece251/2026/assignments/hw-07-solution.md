...
*   **A. Hardware Overwrite:** The Return Address register (`$ra`) is definitively destroyed (overwritten). The `jal` execution natively and forcefully saves the address of the immediately following instruction (`PC + 4`) directly into `$ra`. Because the procedure calls `jal` a *second* time (the recursive act) without shifting that initial return branch offset into the stack, the old return marker is erased forever.
*   **B. The `jr $ra` Failure:** When the recursion bottoms out into the base case and attempts to unfold, the code executes `jr $ra`. However, because the `$ra` points to the recursive block inside itself (the result of the second `jal`) rather than the original `main` entry marker, the program enters an infinite structural loop. It will continually bounce between `jr $ra` and the inner recursively executed loop, never returning dynamically to `main`. This generates a fatal stack fault or timeout.
