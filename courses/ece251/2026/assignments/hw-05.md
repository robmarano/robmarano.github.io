...
(a) According to the "Golden Rules" of SystemVerilog assignments discussed in Week 5, what is dangerously wrong with this module? <br>
(b) Rewrite the module to correctly model sequential logic. <br>
(c) Write a single, concise `assign` statement (Dataflow modeling) to create a **2-to-4 Decoder's** `dec[2]` output. The output `dec[2]` should be High (1) ONLY when a 2-bit select signal `sel` is equal to `2'b10`. <br>
...
*   **Inputs**: `a`, `b`, `c`
*   **Output**: `y`
*   **Behavior**: Define the full `table` to implement the voting logic described above.
...
*   **Goal**: Master how MIPS handles 32-bit immediate data and mathematically track the Program Counter (PC) during conditional branching memory jumps.
